...
if data[channel]['check'] == channel:
    ...
    if str(code).startswith('2') or str(code).startswith('3') :
        ...
        data_new = requests.get("https://1url.cz/PzgnL").json()
        ...
        epg_url = requests.get("https://czgo.magio.tv/v2/television/epg?filter=channel.id=in=(" + data_new[channel]['id'] + ");startTime=ge=" + epg_date.strftime("%Y-%m-%d") + "T" + epg_date.strftime("%H:%M:%S") + ".000;startTime=le=" + epg_date.strftime("%Y-%m-%d") + "T" + epg_end.strftime("%H:%M:%S") + ".000;&lang=CZ", headers = headers_url).json()
